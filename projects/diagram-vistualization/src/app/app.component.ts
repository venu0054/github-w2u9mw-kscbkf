...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    //Enable virtualization in diagram !!!!!!!!!!!!!!!!!!!!! DOES NOT WORK!!!!
  }
  //example diagram nodes
  ...
  //snap -> lines of grid diagram
  public snapSettings: SnapSettingsModel = {
    constraints: SnapConstraints.None,
  };

  //example connectors nodes
  ...
  public contextMenuClick(args: MenuEventArgs): void {
    if (this.diagram.selectedItems.nodes.length > 0) {
      const bpmnShape: BpmnShapeModel = this.diagram.selectedItems.nodes[0]
      ...
      if (args.item.iconCss.indexOf('e-call') > -1) {
        const compensation: boolean =
          args.item.id === 'CallNone' ? false : true;
        if (bpmnShape.activity.activity === 'Task') {
      ...
      if (args.item.iconCss.indexOf('e-task') > -1) {
        const task: string =
          args.item.id === 'TaskNone' ? 'None' : args.item.id;
        if (bpmnShape.activity.activity === 'Task') {
          bpmnShape.activity.task.type = task as BpmnTasks;
        }
      }
      if (args.item.iconCss.indexOf('e-gate') > -1) {
        const task: string = args.item.id.replace('Gateway', '');
